...
class PublisherInstance:
    ...
    def _get_max_photos(self, max_photos_raw) -> int:
        """Приводит max_photos к числу, обрабатывая разные типы"""
        ...
        return max(1, min(max_photos, 10))
    
    def publish_single_folder(self, folder_name: str, ad_text: str, 
                              metadata_text: str, images_data: List,
                              max_photos: int = 10) -> Tuple[bool, str]:
        """
        Публикует одну папку
        
        Args:
            folder_name: имя папки
            ad_text: текст объявления
            metadata_text: метаданные (после #изъятая)
            images_data: список изображений
            max_photos: максимальное количество фото
        """
        try:
            # Приводим max_photos к числу
            max_photos = self._get_max_photos(max_photos)
            ...
            logger.info(f"🖼️ [ПУБЛИКАЦИЯ] Количество изображений: {len(images_data) if isinstance(images_data, list) else 0}")
            logger.info(f"📸 [ПУБЛИКАЦИЯ] Максимум фото: {max_photos}")
            ...
            if isinstance(images_data, list):
                max_images = min(len(images_data), max_photos)
            else:
                max_images = 0
            ...
            for i in range(max_images):
                ...
                img_data = images_data[i] if i < len(images_data) else None
                if not img_data:
