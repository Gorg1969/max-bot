...
class Publisher:
    ...
    def publish_single_folder(self, user_id, folder_name, ad_text, metadata_text, images_data):
        ...
        try:
            if self.STOP_FLAG.get(user_id, False):
                ...
                if hasattr(self.db, 'add_publication'):
            ...
            for i in range(max_images):
                ...
                # ИСПРАВЛЕНО: правильная обработка image_data
            ...
            if hasattr(self.db, 'save_ad_metadata'):
    ...
    def extract_chat_id(self, folder_name: str) -> Optional[str]:
        ...
        # Пример: из "1/6 -76868172202744" извлекаем "1"
        import re
    ...
    def _upload_file_to_max(self, image_data, user_id: int) -> Optional[str]:
        ...
        """
        try:
            # ИСПРАВЛЕНО: безопасная обработка image_data
            if isinstance(image_data, dict):
                # Если это словарь с данными изображения
                image_hash = hash(str(image_data.get('data', '')) + str(image_data.get('name', '')))
                return f"token_{image_hash}"
            elif isinstance(image_data, bytes):
                # Если это байты изображения
                return f"token_{hash(image_data)}"
            else:
                # Если это что-то другое
                return f"token_{hash(str(image_data))}"
        except Exception as e:
    ...
    def _send_to_chat(self, chat_id: str, text: str, image_tokens: List[str]) -> Tuple[bool, Optional[str]]:
        ...
        logger.info(f"📤 Отправка в чат {chat_id} с {len(image_tokens)} фото")
    ...
    def _send_to_user(self, user_id: int, text: str, image_tokens: List[str]) -> Tuple[bool, Optional[str]]:
        ...
        logger.info(f"📤 Отправка пользователю {user_id} с {len(image_tokens)} фото")
        return True, None
